...
- **Media.** `MediaBrowser` (`mode: 'manage' | 'pick'`) is the one way media is browsed: the Media page, every "Choose from library" dialog, and the link dialog's File tab. The server does the searching, kind filtering, sorting and paging (`GET /api/media?q=&kinds=&sort=&limit=&offset=`, 60 per page, with `meta.counts` per kind) and returns `references` for each file, for signed-in users only, so cards can say "Used on 2 pages". A field looks its one file up with `GET /api/media/by-path?path=`.
